                    ...
                    <div class="widget-main padding-2 table-responsive">
                        <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                            ...
                            <?php
                                $no = 1;
                                foreach ($nav as $row) {
                                    $link = ($row['a']['link_nav'] == '1') ? $row['a']['url_nav'] : site_url($row['a']['url_nav']);
                            ?>
                                <tr>
                                    <td><?= ctk($no); ?></td>
                                    <td><?= ctk($row['a']['judul_nav']); ?></td>
                                    <td class="bolder"><?= ($row['a']['parent_nav'] == '0') ?  'Menu Utama' : $row['b']['judul_nav']; ?></td>
                                    <td>
                                        <a target="_blank" href="<?= $link ?>"><?= $row['a']['url_nav'] ?></a>
                                    </td>
                                    <td>
                                        <?= ($row['a']['status_nav'] == '0') ? '<span class="label label-danger label-white arrowed">Tidak Aktif</span>' : '<span class="label label-success label-white arrowed">Aktif</span>' ?>
                                    </td>
                                    <td><?= ctk($row['a']['order_nav']); ?></td>
                                    <td><i class="orange bigger-130 <?= ($row['a']['icon_nav']) ?>"></i></td>
                                    <td nowrap>
                                        <div class="action-buttons">
                                            <a href="<?= site_url($module .'/edit/'. encode($row['a']['id_nav'])) ?>" class="tooltip-warning btn btn-white btn-warning btn-sm" data-rel="tooltip" title="Ubah Data">
                                                    <span class="orange"><i class="ace-icon fa fa-pencil-square-o bigger-130"></i></span>
                                                </a>
                                            <a href="#" name="<?= encode($row['a']['id_nav']) ?>" itemprop="<?= $row['a']['judul_nav'] ?>" id="delete-btn" class="tooltip-error btn btn-white btn-danger btn-sm" data-rel="tooltip" title="Hapus Data">
