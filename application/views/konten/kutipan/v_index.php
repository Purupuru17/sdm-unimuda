                ...
                <div class="widget-body">
                    <div class="widget-main padding-2 table-responsive">
                        <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                            ...
                            <?php
                                $no = 1;
                                foreach ($kutipan['data'] as $row) {
                            ?>
                                <tr>
                                    <td><?= ctk($no); ?></td>
                                    <td><?= ctk($row['oleh']); ?></td>
                                    <td><?= ctk($row['quote']); ?></td>
                                    <td><?= format_date($row['update_kutipan'], 0); ?></td>
                                    <td nowrap>
                                        <div class="action-buttons">
                                            <a href="<?= site_url($module .'/edit/'. encode($row['id_kutipan'])) ?>" class="tooltip-warning btn btn-white btn-warning btn-sm" data-rel="tooltip" title="Ubah Data">
                                                    <span class="orange"><i class="ace-icon fa fa-pencil-square-o bigger-130"></i></span>
                                                </a>
                                            <a href="#" name="<?= encode($row['id_kutipan']) ?>" itemprop="<?= $row['oleh'] ?>" id="delete-btn" class="tooltip-error btn btn-white btn-danger btn-sm" data-rel="tooltip" title="Hapus Data">
