                    ...
                    <div class="widget-main padding-2 table-responsive">
                        <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                            ...
                            ?>
                                <tr>
                                    <td><?= ctk($no); ?></td>
                                    <td><a href="<?= base_url($row['url_file']) ?>" target="_blank">
                                            <strong><?= ctk($row['nama_file']); ?></strong>
                                        </a>
                                    </td>
                                    <td><?= ctk($row['type_file']); ?></td>
                                    <td><?= ctk($row['size_file']); ?></td>
                                    <td nowrap>
                                        <div class="action-buttons">
                                            <button name="<?= base_url($row['url_file']) ?>" id="copy-btn" class="tooltip-info btn btn-white btn-primary btn-sm" data-rel="tooltip" title="Copy Link">
                                                <span class="blue"><i class="ace-icon fa fa-copy bigger-200"></i></span>
                                            </button>
                                            <a href="<?= site_url($module .'/edit/'. encode($row['id_file'])) ?>" class="tooltip-warning btn btn-white btn-warning btn-sm" data-rel="tooltip" title="Ubah Data">
                                                    <span class="orange"><i class="ace-icon fa fa-pencil-square-o bigger-130"></i></span>
                                                </a>
                                            <a href="#" name="<?= encode($row['id_file']) ?>" itemprop="<?= $row['nama_file'] ?>" id="delete-btn" class="tooltip-error btn btn-white btn-danger btn-sm" data-rel="tooltip" title="Hapus Data">
...
<script type="text/javascript">
    $(document.body).on("click", "#copy-btn", function(event) {
