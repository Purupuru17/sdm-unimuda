                    ...
                    <div class="widget-main padding-2 table-responsive">
                        <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                            ...
                            ?>
                                <tr>
                                    <td><?= ctk($no); ?></td>
                                    <td><?= ctk($row['judul_page']); ?></td>
                                    <td><?= ctk($row['slug_page']); ?></td>
                                    <td>
                                        <?= ($row['status_page'] == '0') ? '<span class="label label-danger label-white arrowed">Tidak Aktif</span>' : '<span class="label label-success label-white arrowed">Aktif</span>' ?>
                                    </td>
                                    <td><?= limit_text($row['isi_page'], 150) ?></td>
                                    <td nowrap>
                                        <div class="action-buttons">
                                            <a href="<?= site_url($module .'/edit/'. encode($row['id_page'])) ?>" class="tooltip-warning btn btn-white btn-warning btn-sm" data-rel="tooltip" title="Ubah Data">
                                                    <span class="orange"><i class="ace-icon fa fa-pencil-square-o bigger-130"></i></span>
                                                </a>
                                            <a href="#" name="<?= encode($row['id_page']) ?>" itemprop="<?= $row['judul_page'] ?>" id="delete-btn" class="tooltip-error btn btn-white btn-danger btn-sm" data-rel="tooltip" title="Hapus Data">
